...
export const MobileNav = () => {
  ...
  // 1. Hide on Intro Page and Admin Routes
  if (pathname === '/intro' || pathname.startsWith('/admin')) {
    return null;
  }
  ...
  return (
    <div className="fixed bottom-6 left-0 right-0 z-50 md:hidden flex justify-center px-4">
      ...
      <div className="flex items-center justify-between w-full max-w-sm h-16 px-2 bg-white/95 backdrop-blur-xl border border-black/10 rounded-full shadow-2xl shadow-gray-300/50">
        ...
        {/* 2. Search (Redirects to new /search page) */}
        <NavItem to="/search" icon={Search} label="Search" />
        
        {/* 3. Add (Inline, turns blue only when active) */}
        <Link 
          to="/upload-model" 
          className={cn(
            "flex flex-col items-center justify-center h-full w-full gap-1 transition-all duration-300 relative",
            pathname === "/upload-model" ? "text-blue-500" : "text-muted-foreground"
          )}
        >
          <Plus 
            strokeWidth={pathname === "/upload-model" ? 3 : 2}
            className={cn(
              "w-7 h-7 transition-all duration-300",
              pathname === "/upload-model" ? "scale-110" : ""
            )} 
          />
          <span className="text-[10px] font-medium tracking-tight">Add</span>
        </Link>
