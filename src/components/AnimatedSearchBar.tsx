...
// // export const AnimatedSearchBar = ({ className, autoFocus = false }: AnimatedSearchBarProps) => {
// //   const [index, setIndex] = useState(0);
// //   const [query, setQuery] = useState("");
// //   const navigate = useNavigate();
...
// //     e.preventDefault();
// //     if (query.trim()) {
// //         // Navigate to the explorer page with search query
// //         navigate(`/explorer?search=${encodeURIComponent(query)}`);
// //     }
// //   };

// //   return (
// //     <form onSubmit={handleSearch} className={`relative group w-full ${className}`}>
// //       {/* Glow Effect */}
// //       <div className="absolute -inset-0.5 bg-gradient-to-r from-primary via-blue-500 to-purple-600 rounded-full opacity-30 group-hover:opacity-60 transition duration-500 blur-md"></div>
...
// //             type="text" 
// //             autoFocus={autoFocus}
// //             className="absolute inset-0 w-full h-full border-0 bg-transparent focus-visible:ring-0 text-base md:text-lg z-20 placeholder:text-transparent"
// //             value={query}
// //             onChange={(e) => setQuery(e.target.value)}
// //           />
...
//   defaultValue?: string; // New Prop to persist query
// }
...
//     if (onSearch) {
//       // If a custom handler is provided (e.g., on SearchPage), use it
//       onSearch(query);
//     } else {
//       // Default behavior: Navigate to Explorer (e.g., on Home Page)
//       navigate(`/explorer?search=${encodeURIComponent(query)}`);
//     }
//   };

//   // Trigger search on clear (optional, if you want to reset results when empty)
//   useEffect(() => {
//     if (onSearch && query === '') {
//         onSearch('');
//     }
