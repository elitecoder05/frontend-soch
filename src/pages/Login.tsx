...
export const Login = () => {
  ...
  const location = useLocation();
  const { toast } = useToast();
  const { login } = useAuth(); // ✅ No need for updateAuthState here
  const [isLoading, setIsLoading] = useState(false);
  const [isGoogleLoading, setIsGoogleLoading] = useState(false);
  const [showPassword, setShowPassword] = useState(false);
  const [formData, setFormData] = useState<LoginData>({
  ...
  const getRedirectPath = () => {
    ...
    if (stateFrom?.pathname) return stateFrom.pathname;
    return '/';
  };
  ...
  const processGoogleSignInResult = async (user: any) => {
    try {
      ...
      if (data.success && data.data.user && data.data.token) {
        // ✅ FIX: Synchronous login. No setTimeout, no extra API calls.
        login(data.data.user, data.data.token);
  ...
  useEffect(() => {
    const handleRedirectResult = async () => {
      try {
        ...
        console.error('Google redirect result error:', error);
        // Ignore "No redirect result" error as it happens on normal page loads
        if (error.message && !error.message.includes('No redirect result available')) {
          toast({
            title: 'Google Sign-in Failed',
            description: error.message,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success && response.data.user && response.data.token) {
        // ✅ FIX: Update Context synchronously
        login(response.data.user, response.data.token);
